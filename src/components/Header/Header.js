...
16 -58 -11z"
    ...
    <Div2>
      {navItems.map((item) => (
        <li key={item.href} style={{ zIndex: "5", padding: "0 12px" }}>
          <Link href={item.href}>
    ...
    <MenuToggle onClick={() => setIsOpen((prev) => !prev)} aria-label="Toggle menu">
    ...
    {isOpen && (
      <MobileMenu>
        {navItems.map((item) => (
          <li key={item.href}>
            <Link href={item.href}>
              <MobileNavLink onClick={() => setIsOpen(false)}>{item.label}</MobileNavLink>
            </Link>
          </li>
        ))}
      </MobileMenu>
